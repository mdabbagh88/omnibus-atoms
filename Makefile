...
purge:
	...
	rm -rf /var/cache/atoms-server/src/atoms-server
	# Force a new download of Curl's certificate bundle because it gets updated
	# upstream silently once every while
	rm -rf /var/cache/atoms-server/cache/cacert.pem
	# Clear out old packages to prevent uploading them a second time to S3
	rm -rf /var/cache/atoms-server/pkg
	mkdir -p pkg
	(cd pkg && find . -delete)

# Instead of pkg/atoms-server-xxx.deb, put all files in pkg/ubuntu/atoms-server-xxx.deb
