...
PACKAGECLOUD_USER=atomsd
PACKAGECLOUD_REPO:=$(shell support/repo_name.sh)
PACKAGECLOUD_OS:=$(shell bundle exec support/ohai-helper repo-string)
ifeq ($(shell support/is_atoms_ee.sh; echo $$?), 0)
RELEASE_PACKAGE=atoms-ee
else
RELEASE_PACKAGE=atoms-ce
endif
...
do_rpi2_release: no_changes purge test_build move_to_platform_dir
...
# Instead of pkg/atoms-xxx.deb, put all files in pkg/ubuntu/atoms-xxx.deb
...
move_to_secret_dir:
	if support/is_atoms_ee.sh ; then \
	  mv pkg ${SECRET_DIR} \
	  && mkdir pkg \
	  && mv ${SECRET_DIR} pkg/ \
	  ; fi
...
docker_push:
	docker tag -f $(RELEASE_PACKAGE):latest atoms/$(RELEASE_PACKAGE):$(DOCKER_TAG)
	docker push atoms/$(RELEASE_PACKAGE):$(DOCKER_TAG)

docker_push_latest:
	# push as :rc tag first, if it's stable it overwrites the :rc
	docker tag -f $(RELEASE_PACKAGE):latest atoms/$(RELEASE_PACKAGE):rc
	docker push atoms/$(RELEASE_PACKAGE):rc

ifeq (,$(findstring rc,$(RELEASE_VERSION)))
	# push as :latest tag, the :latest is always the latest stable release
	docker tag -f $(RELEASE_PACKAGE):latest atoms/$(RELEASE_PACKAGE):latest
	docker push atoms/$(RELEASE_PACKAGE):latest
endif
