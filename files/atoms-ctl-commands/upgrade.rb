...
add_command 'upgrade', 'Run migrations after a package upgrade', 1 do |cmd_name|
  ...
  service_statuses = `#{base_path}/bin/atoms-ctl status`

  if /: runsv not running/.match(service_statuses) || service_statuses.empty? then
    log 'It looks like Atoms has not been installed yet; skipping the upgrade '\
      'script.'
    exit! 0
  end

  log 'Shutting down all Atoms services except those needed for migrations'
  ...
  log 'Reconfiguring Atoms to apply migrations'
  reconfigure(false) # sending 'false' means "don't quit afterwards"

  log 'Restarting previously running Atoms services'
  get_all_services.each do |sv_name|
...
Upgrade complete! If your Atoms server is misbehaving try running
